...
**Table 3.** Live QGC acceptance results, from v1.5.0 validation. Both test environments passed — same-phone (on-device loopback) and desktop QGroundControl over Wi-Fi — and testing was repeated across several Android OS versions. The sustained-stability run followed the acceptance specification's continuous-telemetry window.
